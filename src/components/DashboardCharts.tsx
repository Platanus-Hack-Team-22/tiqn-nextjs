  ...
  return (
    <div className="glass-card flex flex-col rounded-lg p-4">
      <div className="mb-2 flex items-center justify-between">
        <span className="font-mono text-[10px] tracking-widest text-cyan-500/70 uppercase">
          Net_Traffic
        </span>
        <span className="font-mono text-[10px] text-cyan-400">
          {data.length > 0 ? Math.round(data[data.length - 1]!) : 0} MB/s
        </span>
      </div>
      <div className="relative h-24 w-full overflow-hidden rounded border border-cyan-500/10 bg-slate-900/50">
        <svg
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setStats((prev) => ({
        ...
        police: Math.min(
          100,
          Math.max(20, prev.police + (Math.random() - 0.5) * 15),
        ),
        fire: Math.min(
          100,
          Math.max(10, prev.fire + (Math.random() - 0.5) * 5),
        ),
      }));
    }, 1000);
    return () => clearInterval(interval);
    ...
    <div className="glass-card flex flex-col rounded-lg p-4">
      ...
      <div className="relative flex h-24 items-center justify-center">
        <svg className="h-full w-full" viewBox="0 0 100 100">
          ...
          {/* Data Shape */}
          <polygon
            points={`${firePoint} ${policePoint} ${emsPoint}`}
            fill="rgba(6,182,212,0.3)"
            stroke="rgba(6,182,212,0.8)"
            strokeWidth="1.5"
          >
            <animate attributeName="points" dur="1s" />
          </polygon>
          ...
          <text
            ...
            y="8"
            textAnchor="middle"
            className="fill-red-400 font-mono text-[6px]"
          >
            FIRE
          </text>
          <text
            ...
            y="75"
            textAnchor="middle"
            className="fill-blue-400 font-mono text-[6px]"
          >
            POLICE
          </text>
          <text
            ...
            y="75"
            textAnchor="middle"
            className="fill-emerald-400 font-mono text-[6px]"
          >
            EMS
          </text>
        </svg>
...
function SystemStatus() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCpu((prev) =>
        Math.min(100, Math.max(0, prev + (Math.random() - 0.5) * 20)),
      );
      setMemory((prev) =>
    ...
    <div className="glass-card flex flex-col rounded-lg p-4">
      <div className="mb-2 flex items-center justify-between">
        ...
        <div className="flex gap-1">
          <div
            className={`h-1.5 w-1.5 rounded-full ${cpu > 80 ? "animate-pulse bg-red-500" : "bg-green-500"}`}
          />
      ...
      <div className="grid h-24 grid-cols-2 gap-2">
        <CircularGauge
          label="AI_CORE"
          value={cpu}
          color={cpu > 80 ? "text-red-400" : "text-cyan-400"}
          stroke={cpu > 80 ? "stroke-red-500" : "stroke-cyan-500"}
        />
        <CircularGauge
          label="DB_IO"
          value={memory}
